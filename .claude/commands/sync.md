...
```
@jogi/extract dirty (N files, M LOC). Commit + push? (y / n / w)
...
INSTALLED_SHA=$(grep -oE 'jogi-extract#[a-f0-9]{40}' ../jogi/package-lock.json | head -1 | grep -oE '[a-f0-9]{40}')
```
...
```bash
cd ../jogi
npm install @jogi/extract@github:luvidal/jogi-extract#$LOCAL_SHA --legacy-peer-deps
npx tsc --noEmit
```
