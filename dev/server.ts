...
function configureExtractor(): void {
...
async function listDoctypes(_req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    configureExtractor()
    const doctypes = JSON.parse(fs.readFileSync(DOCTYPES_PATH, 'utf8')) as DoctypesMap
    const ids = Object.keys(doctypes).map(id => ({ id, label: doctypes[id].label ?? id }))
    sendJson(res, 200, { doctypes: ids })
}

async function extractUploads(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
    ...
    for (const file of files) {
        ...
        const doctype = file.doctype ?? body.doctype
        if (!doctype) {
            results.push({ path: file.path || file.name, error: 'doctype is required', durationMs: 0 })
            continue
        }
        try {
            const buffer = Buffer.from(file.data, 'base64')
            const mimetype = mimetypeFor(file.name, file.type)
            const result = await extract(buffer, mimetype, doctype, {
                ...(body.model ? { model: body.model } : {}),
                ...(body.generationConfig ? { generationConfig: body.generationConfig } : {}),
            })
            results.push({ path: file.path || file.name, mimetype, doctype, durationMs: Date.now() - started, ...result })
        } catch (err) {
...
const server = http.createServer((req, res) => {
    if (req.method === 'GET' && req.url === '/api/doctypes') {
        listDoctypes(req, res).catch(err => sendJson(res, 500, { error: String(err instanceof Error ? err.message : err) }))
        return
    }
    if (req.method === 'POST' && req.url === '/api/extract') {
        extractUploads(req, res).catch(err => sendJson(res, 500, { error: String(err instanceof Error ? err.message : err) }))
        return
    }
    if (req.method === 'GET') {
        serveStatic(req, res)
